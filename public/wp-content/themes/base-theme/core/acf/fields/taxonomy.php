	...
	class acf_field_taxonomy extends acf_field {
		...
		*/
		...
		function load_value( $value, $post_id, $field ) {
			...
			if ( $field['load_terms'] ) {
				...
				// get terms
				$term_ids = wp_get_object_terms( $post_id, $field['taxonomy'], array( 'fields'  => 'ids',
				                                                                      'orderby' => 'none'
				) );
		...
		*/
		...
		<div <?php acf_esc_attr_e( $div ); ?>>
			<?php if ( $field['add_term'] && current_user_can( $taxonomy->cap->manage_terms ) ): ?>
				<a href="#" class="acf-icon acf-icon-plus acf-js-tooltip small acf-soh-target" data-name="add"
				   title="<?php echo sprintf( __( 'Add new %s ', 'acf' ), $taxonomy->labels->singular_name ); ?>"></a>
			<?php endif;
		...
		*/
		...
		function ajax_add_term() {
			...
			?>
			<p class="acf-submit">
			<button class="acf-button blue" type="submit"><?php _e( "Add", 'acf' ); ?></button>
			<i class="acf-spinner"></i><span></span></p></form><?php
