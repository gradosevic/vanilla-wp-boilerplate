...
<div id="acf-upgrade-wrap" class="wrap">
	...
	<table class="wp-list-table widefat">
		...
		<tr>
			...
			<th class="manage-column" scope="col" style="width:33%;"><label
					for="sites-select-all"><?php _e( "Site", 'acf' ); ?></label></th>
		...
		<tr>
			...
			<th class="manage-column" scope="col"><label for="sites-select-all-2"><?php _e( "Site", 'acf' ); ?></label>
			</th>
			<th><?php _e( "Description", 'acf' ); ?></th>
		</tr>
		</tfoot>
		...
		<?php foreach ( $sites as $i => $site ): ?>
			...
			<tr<?php if ( $i % 2 == 0 ): ?> class="alternate"<?php endif; ?>>
				...
				<td>
					<?php if ( $site['updates'] ): ?>
						<span
							class="response"><?php printf( __( 'Site requires database upgrade from %s to %s', 'acf' ), $site['acf_version'], $plugin_version ); ?></span>
					<?php else: ?>
	...
	<p class="show-on-complete"><?php _e( 'Database Upgrade complete', 'acf' ); ?>. <a
			href="<?php echo network_admin_url(); ?>"><?php _e( "Return to network dashboard", 'acf' ); ?></a>.</p>
